...
class VisionTransformer(nn.Module):
    ...
    # 为每个 Transformer 层生成一个 Adapter，并加入可训练列表
    ...
    def reweight_adapter(self, adapter, idx):
        """
        用 momentum 对历史 adapter 快照做重加权。
        adapter: 当前的 adapter ModuleList
        idx: 当前要融合的快照索引（1-based）
        """
        momentum = self.config.adapter_momentum
        # 如果还没存过任何 adapter 快照，或者 idx=0，就直接返回当前 adapter
        if momentum == 0 or idx == 0 or idx > len(self.adapter_list):
            return adapter

        # 对每一层的 adapter doing EMA 融合
        for layer_idx in range(len(self.blocks)):
            # down_proj.weight
    ...
    def forward_features(self, x, adapter_id, train):
        ...
        if adapter_id == -1:  # 仅推理，不用 Adapter
            x = self.blocks(x)
        else:
...
def vit_base_patch16_224_lkcil(pretrained=False, **kwargs):
    model = VisionTransformer(**kwargs)
    checkpoint = timm.create_model("vit_base_patch16_224", pretrained=True, num_classes=kwargs["num_classes"])
    state_dict = checkpoint.state_dict()
    for k in list(state_dict.keys()):
        if "qkv.weight" in k:
            qkv = state_dict.pop(k)
            state_dict[k.replace("qkv.weight", "q_proj.weight")] = qkv[:768]
            state_dict[k.replace("qkv.weight", "k_proj.weight")] = qkv[768:1536]
            state_dict[k.replace("qkv.weight", "v_proj.weight")] = qkv[1536:]
        if "qkv.bias" in k:
            b = state_dict.pop(k)
            state_dict[k.replace("qkv.bias", "q_proj.bias")] = b[:768]
            state_dict[k.replace("qkv.bias", "k_proj.bias")] = b[768:1536]
            state_dict[k.replace("qkv.bias", "v_proj.bias")] = b[1536:]
    for k in list(state_dict.keys()):
        if "mlp.fc" in k:
            w = state_dict.pop(k)
            state_dict[k.replace("mlp.", "")] = w
...
def vit_base_patch16_224_in21k_lkcil(pretrained=False, **kwargs):
    model = VisionTransformer(**kwargs)
    checkpoint = timm.create_model(
        "vit_base_patch16_224_in21k", pretrained=True, num_classes=kwargs["num_classes"]
    )
    state_dict = checkpoint.state_dict()
    for k in list(state_dict.keys()):
        if "qkv.weight" in k:

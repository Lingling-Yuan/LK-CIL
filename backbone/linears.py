...
# backbone/linears.py  (第一段 import 位置)
try:
    # timm < 0.9
...
class SemanticCosineLinear(nn.Module):
    """
    CosineLinear + Text 原型初始化 + LoRA 低秩微调
    """
    def __init__(self, in_dim, out_dim, r=4, bias=False):
        super().__init__()
        self.weight = nn.Parameter(torch.randn(out_dim, in_dim))
        self.scale  = nn.Parameter(torch.tensor(10.))      # learnable s
        # LoRA 低秩分解
        ...
        self.bias = None if not bias else nn.Parameter(torch.zeros(out_dim))
    ...
    @torch.no_grad()
    def init_with_text(self, text_feats: torch.Tensor, cls_order: list):
        """
        text_feats: {cls_name: (1,D)}
        cls_order : list of class names按顺序
        """
        w = torch.cat([text_feats[c] for c in cls_order], dim=0)
        w = torch.nn.functional.normalize(w, dim=-1)
        self.weight.data.copy_(w)

    def forward(self, x):
        w = self.weight + (self.B @ self.A)                # LoRA 注入
